...
живой отчёт «что находят инструменты», а не гейт.
...
### Обновление пинов инструментов (nightly)

Версии gitleaks/semgrep/osv-scanner пинованы в `actions/*/action.yml` и
дрейфуют от апстрима; какие пины существуют, где лежат и откуда брать
апстрим — спека `.github/pins.json`. Ночной workflow `nightly-bump`
(`.github/workflows/nightly-bump.yml`) снимает свежие версии,
прогоняет их на тех же негатив-фикстурах, что и selftest (эталонные находки
обязаны ловиться), и открывает PR `bump/tools` с таблицей «было → стало».

У `osv-scanner` вместе с версией переезжает и пин `sha256`: апстрим файла
сумм не публикует, поэтому ожидаемая сумма живёт здесь, в `action.yml`, и
ревьюится как код. Если версия **не** менялась, а сумма ассета другая —
ночь падает с явным сообщением и пин не обновляется: это ровно тот случай,
ради которого пин и заведён (#13).

Политика мержа: **минорные бампы мержатся пачкой** без разбора; **мажорные —
только после разбора changelog апстрима** (PR помечен `MAJOR` в заголовке).
selftest на бамп-PR сам не стартует (PR создан `GITHUB_TOKEN`) — валидация
уже выполнена в самом nightly-прогоне, ссылка в теле PR; при желании
selftest дёргается вручную. Тег `v1` двигается отдельно, как обычно.

Исключение — **semgrep**: пин помечен `autobump: false` в
`.github/pins.json` (в `action.yml` лежит только сама версия) и ночью не
двигается, потому что зеркалится в образе фермы `polygon-iac`
(`semgrep==…` в роли `base`); переезд `v1` с новым пином без пересборки
шаблона роняет `sast-semgrep` у всех потребителей на ферме. Ночь только
заводит issue о дрейфе; пин и роль поднимаются руками одним заходом —
сначала шаблон, потом тег.
...
.github/workflows/
  ...
  nightly-bump.yml     # ночной бамп пинов: кандидат на негатив-фикстурах → PR bump/tools
  tag-lag.yml          # еженедельный сторож отставания тега v1 от main (issue через health-issue)
.github/pins.json      # спека пинов для ночного бампа: файл, вход, источник апстрима, autobump
.github/scripts/pr-upsert.sh   # PR ночного бампа: создать или обновить; политика, транзиент и «уже есть» различаются
actions/               # composite actions: по одной на инструмент + служебные
  gitleaks/ · semgrep/ (+rules/) · osv-scanner/     # стадии
  sarif-report/ · profile-resolve/                  # приёмник и матрица режимов
  fetch-verified/ · health-issue/                   # загрузка со сверкой, признак живости
  pin-discover/ · pin-apply/ · pin-tools/pins.py    # механика ночного бампа: снять пины и апстрим → записать цели
tests/
  negative/            # «гард обязан отказать»: фикстуры на каждый инструмент
  lint/                # детекторы гигиены манифестов и расхождения близнецов
  health-issue/        # логика признака живости на заглушке gh
