...
# Все загрузки идут ОДНОЙ функцией, и это не косметика. Прежняя форма
# `curl -sfL` несла два дефекта сразу, и ночь 14.09 предъявила оба
# (devsecops-pipeline#70):
#
#   -s без -S    отказ молчит. В логе осталось «положительный контроль
#                упал (rc=22)» — ни URL, ни кода ответа, а 22 у curl это
#                ЛЮБОЙ HTTP >= 400. Разбирать было не по чему.
#   без --retry  транзиент раздатчика релизных ассетов роняет джобу
#                целиком. Тот же коммит на том же раннере через полтора
#                часа прошёл зелёным (прогон 34824617790 против
#                34833999237) — то есть падало не то, что проверяется.
#
# --retry БЕЗ --retry-all-errors — выбор, а не небрежность. Повторяется
# ровно транзиентный класс (таймаут, 408, 429, 5xx); 404 отвергается
# сразу, потому что переименованный или снятый апстримом ассет обязан
# краснеть немедленно — повтор его не вернёт, а настоящую находку
# задержит. Замер локальным сервером 2026-09-14: 503,503,200 -> 3
# запроса, rc=0, 2 c; 404 -> 1 запрос, rc=22, 0 c. Оба случая держит
# tests/negative/assert-fetch-verified.sh (18 и 19), потому что повтор,
# который никто не проверял, — это повтор, который завтра станет
# `--retry-all-errors` и проглотит 404.
#
# Сверки повтор не касается: сумма и подпись считаются по тому, что в
# итоге приехало, а не по номеру попытки.
dl() { # <url> <куда> <что это>
  local url="$1" out="$2" what="$3" rc=0
  curl -fL -sS --retry 3 --retry-delay 2 --retry-connrefused \
       --connect-timeout 15 --max-time 600 \
       -o "$out" "$url" 2>"$WORK/curl.err" || rc=$?
  if [ "$rc" -eq 0 ]; then
    return 0
  fi
  # Отказ ЗАГРУЗКИ не имеет права выглядеть как отказ СВЕРКИ: за первым
  # идут к апстриму и в сеть, за вторым — разбирать подмену. Восемь ночей
  # одинакового красного этот портфель уже оплатил на #44.
  echo "::error::fetch-verified: не скачался $what — отказ ЗАГРУЗКИ, не сверки (curl rc=$rc)" >&2
  echo "  $url" >&2
  sed 's/^/  /' "$WORK/curl.err" >&2
  exit "$rc"
}
