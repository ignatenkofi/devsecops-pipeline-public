...
def scanned_count(report: Path) -> int:
    ...
    является: на semgrep 1.172.0 и 1.177.0 ключ присутствует всегда
    (проверено и на применимом, и на неприменимом репозитории), поэтому
    его пропажа означает смену контракта, а не отсутствие находок.
    """
